...
def main(args):
    ...
    for epoch in range(args.epochs):
        ...
        for i, (inputs, targets) in enumerate(train_loader):
            ...
            loss_kd = F.kl_div(log_student, sof_teacher, reduction="batchmean") * ( temperature * temperature)
            
            #loss_kd = criterion_kd(logits, teacher_logits)
            loss_ce =  F.cross_entropy(logits, targets)
            
            #loss_vector_reshape = torch.reshape(loss_vector, (-1, 1))

            #Temp=1 
            #loss_KD_vector =nn.KLDivLoss(reduction='none')( F.log_softmax(logits/ Temp, dim=1),  F.softmax(teacher_logits / Temp, dim=1))
            
            #loss_CE = torch.mean(loss_vector_reshape )
            #loss_KD= torch.mean( loss_KD_vector)

            a=0.5
            loss = a*loss_ce+ (1-a)*loss_kd
            
            #loss = criterion_kd(nn.functional.log_softmax(x2/T, dim=1), nn.functional.softmax(t2/T, dim=1))
        ...
        val_loss_, val_acc1_, val_acc5_ = cifar_100_train_eval_loop( args, logger, epoch, optimizer_T, scheduler_T, network_T, valid_loader, criterion, args.eval_batch_size, mode='eval' )
